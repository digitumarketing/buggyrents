...
/* llms.txt exists and every link in it resolves.
 *
 * WHY THIS EXISTS
 * Added 7 Sep 2026 with the file itself. llms.txt is the one document on this site
 * written to be read by machines that will repeat what it says, so a dead link in it
 * does not degrade gracefully the way a dead link on a page does: an assistant reads
 * a URL, tells a guest to go there, and the guest lands on a 404 believing the
 * business sent them.
 *
 * The link audit cannot see any of this. It reads <a href> out of built HTML, and
 * llms.txt is plain text with bare URLs, so every link in it was unchecked by anything
 * until this ran.
 *
 * It also checks the price-basis lines are present. Those two sentences are the whole
 * reason the file is safe to publish: buggies are per vehicle, shared safaris are per
 * person, and an assistant that quotes one basis for the other sends a guest to the
 * base expecting a different bill. If a refactor drops them, the file becomes a
 * liability rather than an asset, and it should stop the deploy.
 */
{
  const file = join(DIST, 'llms.txt');
  if (!existsSync(file)) {
    console.error('llms.txt was not built.');
    console.error('  It is generated by src/pages/llms.txt.ts. See docs/SEO-PLAN.md on GEO.');
    process.exit(1);
  }
  ...
  const required = [
    ['priced **per vehicle**, not per person', 'the per-vehicle price basis'],
    ['Shared desert safari packages are priced **per person**', 'the per-person safari basis'],
    ['Minimum age is 14', 'the age floor']
  ];
  const missingFacts = required.filter(([needle]) => !txt.includes(needle)).map(([, what]) => what);
  if (missingFacts.length) {
    console.error('llms.txt is missing statements that make it safe to publish:');
    missingFacts.forEach(m => console.error('  ' + m));
    console.error('  An assistant quoting a price without its basis misprices every booking.');
    process.exit(1);
  }

  /* Resolve every internal URL the same way a reader would: as a built page. */
  const urls = [...new Set([...txt.matchAll(/https:\/\/buggyrents\.com(\/[^\s)\]]*)/g)].map(m => m[1]))];
  const dead = [];
  for (const path of urls) {
    if (path === '/llms.txt' || path === '/sitemap-index.xml' || path === '/robots.txt') continue;
    ...
    if (!candidates.some(c => existsSync(c))) dead.push(path);
  }

  if (dead.length) {
    console.error('llms.txt links to pages that were not built:');
    dead.slice(0, 10).forEach(d => console.error('  ' + d));
    if (dead.length > 10) console.error(`  ...and ${dead.length - 10} more.`);
    console.error('  An assistant will read these out to a guest as if they work.');
    process.exit(1);
  }

  console.log(`llms.txt audit passed — ${urls.length} links, all resolve, price basis and age floor stated.`);
}
