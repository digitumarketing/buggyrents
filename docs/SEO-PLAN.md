...
- ~~**`public/llms.txt`**, which does not exist today.~~ **Done 7 Sep 2026, pulled forward
  from October.** Generated by `src/pages/llms.txt.ts` from the site's own data, covering
  the business, the four clusters, every machine and package with its real price and price
  basis, the age rules, pickup areas, guides and contact. Audited: the build fails if a link
  in it dies or the price-basis statements go missing. Pulled forward because the September
  baseline found assistants already out-sending Google organic here.
